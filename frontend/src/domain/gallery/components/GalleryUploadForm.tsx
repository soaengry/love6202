...
import { GALLERY_VALIDATION } from "../gallery.constants";

interface GalleryUploadFormProps {
  onUploadComplete: () => void;
}

export const GalleryUploadForm: FC<GalleryUploadFormProps> = ({
  onUploadComplete,
}) => {
  ...
  const validateFiles = (newFiles: File[]): File[] => {
    ...
    for (const file of newFiles) {
      if (!GALLERY_VALIDATION.ACCEPTED_TYPES.includes(file.type as "image/jpeg" | "image/png")) {
        toast.error(`${file.name}: JPEG, PNG 형식만 가능합니다.`);
        continue;
      }
      if (file.size > GALLERY_VALIDATION.MAX_FILE_SIZE_MB * 1024 * 1024) {
        toast.error(`${file.name}: ${GALLERY_VALIDATION.MAX_FILE_SIZE_MB}MB 이하만 가능합니다.`);
        continue;
      }
      valid.push(file);
    }
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (totalCount > GALLERY_VALIDATION.MAX_UPLOAD_COUNT) {
      toast.error(`최대 ${GALLERY_VALIDATION.MAX_UPLOAD_COUNT}장까지 선택 가능합니다.`);
      return;
    }
  ...
  return (
    <div className="gallery-upload-form space-y-4">
      {/* 드래그 앤 드롭 영역 */}
      <div
      ...
      >
        ...
        <p className="text-xs text-text-tertiary mt-1">
          JPEG, PNG · 최대 {GALLERY_VALIDATION.MAX_FILE_SIZE_MB}MB ·{" "}
          {GALLERY_VALIDATION.MAX_UPLOAD_COUNT}장까지
        </p>
        <input
          ref={inputRef}
          type="file"
