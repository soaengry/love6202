...
export const ExtraInfoStep: FC<ExtraInfoStepProps> = ({ control, errors, register }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 드레스코드 */}
      <div>
        <label className="block text-sm font-medium text-text-primary mb-1.5">드레스코드</label>
        <input
      ...
      {/* 주차 안내 */}
      <div>
        <label className="block text-sm font-medium text-text-primary mb-1.5">주차 안내</label>
        <textarea
      ...
      {/* 식사 안내 */}
      <div>
        <label className="block text-sm font-medium text-text-primary mb-1.5">식사 안내</label>
        <textarea
      ...
      {/* 안내 / 공지사항 */}
      <div>
        <label className="block text-sm font-medium text-text-primary mb-1.5">안내 문구</label>
        <textarea
          {...register("wedding.notice")}
          placeholder="하객분들께 전하고 싶은 말씀"
          rows={3}
          className="w-full px-4 py-3 border border-border rounded-xl bg-bg-primary text-text-primary focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-colors resize-none"
        />
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <button
            type="button"
            onClick={() =>
              transFields.append({ type: "SUBWAY", title: "", description: "", orderIndex: transFields.fields.length })
            }
        ...
        {transFields.fields.length === 0 && (
          <p className="text-sm text-text-secondary text-center py-4">교통편을 추가해주세요. (선택)</p>
        )}

        {transFields.fields.map((field, index) => (
          <div key={field.id} className="bg-bg-secondary rounded-xl p-4 space-y-3">
            <div className="flex items-center justify-between">
              <input type="hidden" {...register(`transportations.${index}.orderIndex`)} value={index} />
              <select
            ...
            {errors.transportations?.[index]?.title && (
              <p className="mt-1 text-sm text-red-500">{errors.transportations[index].title?.message}</p>
            )}
      ...
      <div className="space-y-3">
        ...
        {announceFields.fields.length === 0 && (
          <p className="text-sm text-text-secondary text-center py-4">공지사항을 추가해주세요. (선택)</p>
        )}
          ...
          <div key={field.id} className="bg-bg-secondary rounded-xl p-4 space-y-3">
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2 text-sm text-text-secondary cursor-pointer">
                <input type="checkbox" {...register(`announcements.${index}.isPinned`)} className="rounded accent-primary" />
                고정
              </label>
              <button
                type="button"
            ...
            {errors.announcements?.[index]?.title && (
              <p className="mt-1 text-sm text-red-500">{errors.announcements[index].title?.message}</p>
            )}
            ...
            {errors.announcements?.[index]?.content && (
              <p className="mt-1 text-sm text-red-500">{errors.announcements[index].content?.message}</p>
            )}
          </div>
        ))}
      </div>
